...
protected:
	virtual void SetUp()
	{
		http_ = new chain::HttpClient();
		http_->Initialize("192.168.10.92:19333");
		//address:chainV8i2MLZd5ahDGay6oAZHiMyYNUkJfSiTAmJy
		priv_key_ = new chain::PrivateKey("privC1CCDoFh9kfbKRHf9PZMPzH1N5vkBZWdbJxnkiZji7UfZsSXvUxU");
	}
...
public:
	static chain::PrivateKey *priv_key_;
	static chain::HttpClient *http_;
};

chain::PrivateKey *ApiTest::priv_key_ = NULL;
chain::HttpClient *ApiTest::http_ = NULL;
Json::Value GetTransactionJson(chain::PrivateKey & prv){
...
Json::Value SubmintTransaction(chain::PrivateKey &prv,std::string sign_data, std::string transaction_blob){
	Json::Value item;
...
bool CheckTransaction(int64_t time_out,std::string hash,chain::HttpClient& http){
	...
	while (utils::Timestamp::HighResolution() - start_time < time_out){
		...
		chain::HttpClient::RecvMessage rec = http.http_request(chain::HttpClient::HTTP_GET, "getTransactionHistory?hash=" + hash, "");
		Json::Value rep;
...
void ApiTestWorker::Run(utils::Thread *thread){
	chain::PrivateKey priv_key("privC1CCDoFh9kfbKRHf9PZMPzH1N5vkBZWdbJxnkiZji7UfZsSXvUxU");
	chain::HttpClient http;
	...
	while (thread->enabled())
	{
		...
		chain::HttpClient::RecvMessage rec = http.http_request(chain::HttpClient::HTTP_POST, "getTransactionBlob",
		...
		chain::HttpClient::RecvMessage rec1 = http.http_request(chain::HttpClient::HTTP_POST, "submitTransaction",
	...
	while (thread->enabled())
	{
			...
			if (!hash.empty()){
				...
				if (utils::Timestamp::HighResolution() - start_time > time_out){
					...
					chain::HttpClient::RecvMessage rec = http.http_request(chain::HttpClient::HTTP_GET, "getModulesStatus", "");
					Json::Value rep;
					rep.fromCString(rec.context.c_str());
