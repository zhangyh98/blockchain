...
protected:
	virtual void SetUp()
	{
		priv_key = new chain::PrivateKey(chain::ED25519SIG);
	}
...
TEST_F(PrivateKeyTest,PrivateKey2){
	chain::PrivateKey priv_key2(priv_key->GetBase58PrivateKey());
...
TEST_F(PrivateKeyTest, Verify){
	...
	//执行测试验证
	EXPECT_TRUE(false);
	EXPECT_TRUE(chain::PublicKey::Verify(data, signature, publicKey));
	EXPECT_FALSE(chain::PublicKey::Verify(data + "1", signature, publicKey));
	EXPECT_FALSE(chain::PublicKey::Verify(data, signature + "1", publicKey));
//	EXPECT_FALSE(chain::PublicKey::Verify(data, signature, publicKey + "1"));
	//EXPECT_FALSE(chain::PublicKey::Verify(data, signature,""));
	EXPECT_FALSE(chain::PublicKey::Verify(data, "", publicKey));
	EXPECT_FALSE(chain::PublicKey::Verify("", signature, publicKey));
	...
	publicKey2 += "a";
	EXPECT_FALSE(chain::PublicKey::Verify(data2, signature, publicKey));
	EXPECT_FALSE(chain::PublicKey::Verify(data, signature2, publicKey));
	EXPECT_FALSE(chain::PublicKey::Verify(data, signature, publicKey2));
}

//测试PublicKey类中方法
TEST_F(PrivateKeyTest, PublicKey){
	chain::PublicKey pub;
	EXPECT_TRUE(chain::PublicKey::IsAddressValid(priv_key->GetBase58Address()));
