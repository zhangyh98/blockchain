...
namespace chain{
	...
	bool MsgProcessor::ProcessRcvMessages(const std::string& msg)
	{
		...
		{			
			...
			if (test_warning&WARNING_CHAIN_CRACK_HIGH)
				threshold["chain_crack"] = 100;
			else if (test_warning&WARNING_CHAIN_CRACK_LOW)
				threshold["chain_crack"] = 0;
			else
				threshold["chain_crack"] = 100;

			if (test_warning&WARNING_CHAIN_ATTACK_TIME_HIGH)
				threshold["chain_attack_time"] = 100;
			else if (test_warning&WARNING_CHAIN_ATTACK_TIME_LOW)
				threshold["chain_attack_time"] = 0;
			else
				threshold["chain_attack_time"] = 100;

			if (test_warning&WARNING_CHAIN_ATTACK_CONT_HIGH)
				threshold["chain_attack_counts"] = 100;
			else if (test_warning&WARNING_CHAIN_ATTACK_CONT_LOW)
				threshold["chain_attack_counts"] = 0;
			else
				threshold["chain_attack_counts"] = 100;
		...
		else if (receive["method"].asString() == "chain")
		{
			if (current_request_test_ != CHAIN)
				return true;
			if (CheckChain(msg))
			request_test_result_ = true;
			
			DetermineResponse("chain");		
		}
	...
	bool MsgProcessor::CheckLedger(const std::string& msg)
	{
		...
		Json::Value result_from_chain = latest_ledger[0]["result"];
		...
		switch (current_condition_&(~0x3ff))
		{
			case LEDGER_LATEST_20_LEDGER:
				EXPECT_EQ(receive["error_code"].asInt(), 0);
				{
					...
					for (int k = 0; k < 20; ++k)
					{
						result_from_chain = latest_ledger[k]["result"];
						EXPECT_EQ(blocks[k]["ledger_seq"].asInt(), (result_from_chain["ledger_seq"].asInt()));
						EXPECT_STREQ(blocks[k]["hash"].asCString(), result_from_chain["hash"].asCString())<<"index = "<<k;
					}
				}
			...
			case LEDGER_NO_NUM:
				EXPECT_EQ(receive["error_code"].asInt(), 0);
				{
					...
					Json::Value blocks = result["blocks"];
					EXPECT_EQ(blocks[k]["ledger_seq"].asInt(), result_from_chain["ledger_seq"].asInt());
					EXPECT_STREQ(blocks[k]["hash"].asCString(), result_from_chain["hash"].asCString());
				}				
				break;
			case LEDGER_NUM_0:
			case LEDGER_SEQ_0:
...
#else
		EXPECT_STREQ(property["host_name"].asCString(), "chain65");
#endif
	...
	bool MsgProcessor::CheckUpgrade(const std::string& msg)
	{
		...
		case UPGRADE_NO_URL:
			...
			break;
		case UPGRADE_FILENAME_CHAIN:
		case UPGRADE_FILENAME_CHAIND:
		case UPGRADE_FILENAME_CHAINJSON:
	...
	bool MsgProcessor::CheckChain(const std::string& msg)
	{
		bool ret(true);
		Json::Value receive;
		receive.fromString(msg);
	 ...
	 void WebSocketServer::IncompleteRequestGenerator(bool rqst_or_resp,uint32_t missing_part,Json::Value& msg)
	{
	 ...
	 void WebSocketServer::UpgradeLayout(Json::Value& param)
	 {
		 ...
		 case UPGRADE_NO_FILENAME:			 
			 break;
		 case UPGRADE_FILENAME_CHAIN:
			 item["file_name"] = "chain";
			 break;
		 case UPGRADE_FILENAME_CHAIND:
			 item["file_name"] = "chaind";
			 break;
		 case UPGRADE_FILENAME_SLAVE:
	 ...
	 {
		 //get the latest seq
		 chain::HttpClient::RecvMessage rcv = msg_processor_.http_.http_request(chain::HttpClient::HTTP_GET, "/getLedger", "");;
		 ...
		 case LEDGER_LATEST_20_LEDGER:
		 {
			...
			for (int i = 0; i <20; i++)
			{
				...
				std::string seq = t;
				param = param + seq;
				rcv = msg_processor_.http_.http_request(chain::HttpClient::HTTP_GET, param, "");;
	...
	bool WebSocketServer::VerifyErrorLog(int err_code)
	{
		...
		if (flag)
		{
			...
			case 6:
				...
				break;
			case 8:
				target = "on_response_error -- chain info failed";
				break;
			case 9:
	...
	void WebSocketServer::RequestChainTest()
	{
		printf("**Test Start**this is a test to verify the response of request \"chain\"!\n");
		...
		msg_processor_.SetCurrentRequestTest(CHAIN);
	...
	void WebSocketServer::RequestUpgradeTest()
	{
		...
		uint32_t testconditions[] = { 
			GENERAL_CASE_RESPONSE_ERRCODE_0 | UPGRADE_FILENAME_CHAIN,
			GENERAL_CASE_RESPONSE_ERRCODE_14 | UPGRADE_FILENAME_CHAIND,
			GENERAL_CASE_RESPONSE_ERRCODE_21 | UPGRADE_FILENAME_CHAINJSON,
	...
	void WebSocketServer::WarningTest()
	{
		...
		//					WARNING_CHAIN_CRACK_HIGH, WARNING_CHAIN_ATTACK_CONT_LOW, \
		//	WARNING_CHAIN_CRACK_LOW, WARNING_CHAIN_ATTACK_CONT_HIGH, WARNING_CHAIN_ATTACK_CONT_LOW };
