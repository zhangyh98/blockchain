...
all:
	...
	cmake -DSVNVERSION=$(chain_version) -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_VERBOSE_MAKEFILE=ON ../../src; \
	make -j 4
...
clean:
	if [ -d ./lib ]; then rm -rf bin/chain && rm -rf bin/chaind && cd lib/ && ls | grep -v http |grep -v ed25519 | xargs rm -rf; fi

clean_3rd:
	cd src/3rd && make clean_3rd && cd ../../

clean_build:
	cd build/$(CUR_OS)/; ls | grep -v chain | grep -v one_step_pack.sh  | xargs rm -rf
