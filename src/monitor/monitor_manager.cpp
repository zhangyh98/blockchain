...
namespace chain {
	MonitorManager::MonitorManager() : Network(SslParameter()) {
		...
		request_methods_[monitor::MONITOR_MSGTYPE_CHAIN] = std::bind(&MonitorManager::OnChainStatus, this, std::placeholders::_1, std::placeholders::_2);
	...
	chain::Connection * MonitorManager::CreateConnectObject(chain::server *server_h, chain::client *client_, chain::tls_server *tls_server_h, 
		chain::tls_client *tls_client_h, chain::connection_hdl con, const std::string &uri, int64_t id) {
	...
	bool MonitorManager::OnMonitorHello(protocol::WsMessage &message, int64_t conn_id) {
		...
		do {
			...
			reg.set_blockchain_version(chain::General::CHAIN_VERSION);
			reg.set_data_version(chain::General::MONITOR_VERSION);
	...
	bool MonitorManager::OnChainStatus(protocol::WsMessage &message, int64_t conn_id) {
		monitor::ChainStatus chain_status;
		// Get the status of chain
		GetChainStatus(chain_status);
		...
		// Send the response of chain status
		if (NULL == monitor || !monitor->SendResponse(message, chain_status.SerializeAsString(), ignore_ec)) {
			bret = false;
			LOG_ERROR("Failed to send chain status from ip(%s) (%d:%s)", monitor->GetPeerAddress().ToIpPort().c_str(),
				ignore_ec.value(), ignore_ec.message().c_str());
		}
		return bret;
	}
	...
	bool MonitorManager::GetChainStatus(monitor::ChainStatus &chain_status) {
		...
		monitor::GlueManager *glue_manager = chain_status.mutable_glue_manager();
		...
		monitor::PeerManager *peer_manager = chain_status.mutable_peer_manager();
