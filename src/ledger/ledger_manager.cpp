...
namespace chain {
	...
	bool LedgerManager::CreateGenesisAccount() {
		...
		batch->Put(chain::General::KEY_LEDGER_SEQ, "1");
		batch->Put(chain::General::KEY_GENE_ACCOUNT, Configure::Instance().genesis_configure_.account_);
	...
	void LedgerManager::CreateHardforkLedger() {
		...
		do {
			...
			chain::KeyValueDb *ledger_db = chain::Storage::Instance().ledger_db();
			...
			batch->Put(chain::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
			batch->Put(chain::General::LAST_PROOF, "");

			ValidatorsSet(batch, new_validator_set);

			//Write ledger db
			WRITE_BATCH batch_ledger;
			batch_ledger.Put(chain::General::KEY_LEDGER_SEQ, utils::String::ToString(header->seq()));
	...
	bool LedgerManager::CloseLedger(const protocol::ConsensusValue& consensus_value, const std::string& proof) {
		...
		account_db_batch->Put(chain::General::KEY_LEDGER_SEQ, utils::String::Format(FMT_I64, ledger_seq));
		...
		//proof
		account_db_batch->Put(chain::General::LAST_PROOF, proof);
		account_db_batch->Put(chain::General::STATISTICS, statistics_.toFastString());
		proof_ = proof;
	...
	void LedgerManager::OnRequestLedgers(const protocol::GetLedgers &message, int64_t peer_id) {
		...
		if (ret) {
			chain::WsMessagePointer ws = std::make_shared<protocol::WsMessage>();
	...
	Result LedgerManager::DoTransaction(protocol::TransactionEnv& env, LedgerContext *ledger_context) {
		...
		TransactionFrm::pointer txfrm = std::make_shared<chain::TransactionFrm >(env);
